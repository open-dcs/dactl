[GtkTemplate (ui = "/org/opendcs/dcs/ui/settings-listbox.ui")]
public class Dcsg.SettingsListBox : Gtk.ListBox {

    [GtkChild]
    private Gtk.SizeGroup size_group;

    public signal void request_choices (Dcsg.PropertyBox box, GLib.Type type);

    public signal void new_data (GLib.ParamSpec spec, GLib.Value value);

    public signal void request_data (string uri);

    public void populate (Dcsg.SettingValues svalues) {
        ...
        foreach (GLib.ParamSpec spec in svalues.keys) {
            ...
            if (spec.owner_type.name ().contains ("Cld" ) ||
                spec.owner_type.name ().contains ("Dcs")) {
                ...
                var box = new Dcsg.PropertyBox.from_data (spec, value);
        ...
        set_sort_func ((row1, row2) => {
            ...
            var name1 = (box1 as Dcsg.PropertyBox).lbl_nick.get_text ();
            var name2 = (box2 as Dcsg.PropertyBox).lbl_nick.get_text ();
...
[GtkTemplate (ui = "/org/opendcs/dcs/ui/settings-propertybox.ui")]
public class Dcsg.PropertyBox : Gtk.Box {
    [GtkChild]
    ...
    public void set_dcs_object_choices (Gee.Map<string, Dcs.Object> map) {
        Gtk.TreeIter iter;
        Gtk.ListStore store = combo.get_model () as Gtk.ListStore;
