 ...
 */
public class Dcs.UI.ChannelMatrix : GLib.Object, Dcs.Object,
                            Dcs.Container, Dcs.Buildable, Dcs.CldAdapter {
     ...
     */
    private Gee.Map<string, Dcs.TriplePoint?> _data;
    public Gee.Map<string, Dcs.TriplePoint?> data {
        get {
            return _data ;
        }
        private set {
    ...
    construct {
        ...
        data = new Gee.TreeMap<string, Dcs.TriplePoint?> ();
    }
            ...
            for (Xml.Node *iter = node->children; iter != null; iter = iter->next) {
                if (iter->name == "object"){
                    ...
                    if (type == "channel-matrix-element") {
                        var child = new Dcs.UI.ChannelMatrixElement.from_xml_node (iter);
                        child.id = object_id;
                        add_child (child);
                        data.set (child.chref,
                            new Dcs.TriplePoint () { a = child.a,
                                                          b = child.b, c = 0 });
                    }
                }
            }
    ...
    public void offer_cld_object (Cld.Object object) {
        ...
        foreach (var obj in objects.values) {
            if (obj is Dcs.UI.ChannelMatrixElement) {
                if (!(data.has_key ((obj as Dcs.UI.ChannelMatrixElement).chref))){
                    test = false;
                }
            }
        }
    ...
    protected async void request_data () {
        var elements = get_object_map (typeof (Dcs.UI.ChannelMatrixElement));
        while (!satisfied) {
            foreach (var element in elements.values) {
                Dcs.UI.ChannelMatrixElement e = element as Dcs.UI.ChannelMatrixElement;
