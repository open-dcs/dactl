            ...
            for (Xml.Node *iter = node->children; iter != null; iter = iter->next) {
                if (iter->name == "property") {
                    ...
                    if (type == "tree-category") {
                        var category = new Dcs.UI.ChannelTreeCategory.from_xml_node (iter);
                        add_child (category);
                    } else if (type == "tree-entry") {
                        var entry = new Dcs.UI.ChannelTreeEntry.from_xml_node (iter);
                        add_child (entry);
                    }
                }
            }
...
public class Dcs.UI.ChannelTreeView : Dcs.UI.CompositeWidget, Dcs.CldAdapter {
    ...
    public virtual void offer_cld_object (Cld.Object object) {
        var entries = get_object_map (typeof (Dcs.UI.ChannelTreeEntry));
        foreach (var entry in entries.values) {
            if ((entry as Dcs.UI.ChannelTreeEntry).ch_ref == object.uri) {
                debug ("Assigning channel `%s' to `%s'", object.uri, entry.id);
                (entry as Dcs.UI.ChannelTreeEntry).channel = (object as Cld.Channel);
            }
            satisfied = (entry as Dcs.UI.ChannelTreeEntry).channel_isset;
        }
    ...
    protected async void request_data () {
        while (!satisfied) {
            var entries = get_object_map (typeof (Dcs.UI.ChannelTreeEntry));
            foreach (var entry in entries.values) {
                if (!(entry as Dcs.UI.ChannelTreeEntry).channel_isset)
                    request_object ((entry as Dcs.UI.ChannelTreeEntry).ch_ref);
            }
 ...
 */
    ...
    private void create_treeview () {
        ...
        var categories = get_object_map (typeof (Dcs.UI.ChannelTreeCategory));
        foreach (var category in categories.values) {
            ...
            for (i = 0; i <= columns.size; i++) {
                if (i == 0)
                    treemodel.set (iter, i, (category as Dcs.UI.ChannelTreeCategory).title, -1);
                else
                    treemodel.set (iter, i, null, -1);
            }

            var entries = (category as Dcs.Container).get_object_map (typeof (Dcs.UI.ChannelTreeEntry));
            foreach (var entry in entries.values) {
                Gtk.TreeIter child_iter;
                var channel = (entry as Dcs.UI.ChannelTreeEntry).channel;
    ...
    private void treeview_cursor_changed_cb () {
        ...
        var entry = get_object (selection_id) as Dcs.UI.ChannelTreeEntry;
