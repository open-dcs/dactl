public class Dcs.UI.PolarAxis : GLib.Object, Dcs.Buildable, Dcs.Object {
    /**
    ...
    public Dcs.UI.PolarAxisType axis_type { get; set; }

    /* minimum value if the axis */
    private double _min;
    public double min {
        get {
            if (axis_type == Dcs.UI.PolarAxisType.ANGLE) {
                var value = Dcs.UI.degrees_to_positive (_min);
                if (value == 360)
                    return 0;
                else
                    return value;
    ...
    public double max {
        get {
            if (axis_type == Dcs.UI.PolarAxisType.ANGLE) {
                var value = Dcs.UI.degrees_to_positive (_max);
                if (value == 0)
                    return 360;
                else
                    return value;
    ...
    public double intersect {
        get {
            if (axis_type == Dcs.UI.PolarAxisType.ANGLE) {
                var value = Dcs.UI.degrees_to_positive (_intersect);
                return value;
            } else {
                return _intersect;
            }
                ...
                if (iter->name == "property") {
                    switch (iter->get_prop ("name")) {
                        ...
                        case "polar-axis-type":
                            ...
                            axis_type = Dcs.UI.PolarAxisType.parse (value);
                            break;
                        case "min":
                            value = iter->get_content ();
