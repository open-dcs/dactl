...
public class Dcs.UI.StripChartTrace : GLib.Object, Dcs.Object, Dcs.Buildable {
     ...
     */
    public Dcs.UI.TraceDrawType draw_type { get; set; default = Dcs.UI.TraceDrawType.BAR; }
    ...
    private Gee.List<Dcs.Point> _window = null;
    /**
     * Window of data to draw.
     */
    public Gee.List<Dcs.Point> window {
        get {
            /* Set default data whenever the window has be nullified */
            if (_window == null) {
                _window = new Gee.ArrayList<Dcs.Point> ();
                for (int i = 0; i < _window_size; i++)
                    _window.add (new Dcs.Point (0.0, 0.0));
            }
            return _window;
        }
                ...
                if (iter->name == "property") {
                    switch (iter->get_prop ("name")) {
                        case "buffer-size":
                            ...
                            break;
                        case "draw-type":
                            draw_type = Dcs.UI.TraceDrawType.parse (iter->get_content ());
                            break;
                        case "line-weight":
...
private class Dcs.UI.StripChartCanvas : Dcs.UI.Canvas {

    public weak Dcs.UI.Axis t_axis { get; set; }

    public weak Dcs.UI.Axis y_axis { get; set; }

    private Gee.Map<string, Dcs.Object> _traces;

    public Gee.Map<string, Dcs.Object> traces {
        get { return _traces; }
    ...
    construct {
        ...
        _traces = new Gee.TreeMap<string, Dcs.UI.StripChartTrace> ();
    }
    ...
    private void update_padding () {
        ...
        if ((parent as Dcs.UI.StripChart).flags.is_set (Dcs.UI.ChartFlag.DRAW_TITLE))
            padding_top += 20;

        if (y_axis.flags.is_set (Dcs.UI.AxisFlag.DRAW_LABEL))
            padding_left += 50;

        if (t_axis.flags.is_set (Dcs.UI.AxisFlag.DRAW_LABEL))
            padding_bottom += 40;
    }
    ...
    public override bool draw (Cairo.Context cr) {
        ...
        /* Grid */
        if ((parent as Dcs.UI.StripChart).flags.is_set (Dcs.UI.ChartFlag.DRAW_GRID)) {
             ...
             */
            var grid = new Dcs.UI.StripChartGrid (grid_surface);

            if ((parent as Dcs.UI.StripChart).flags.is_set (Dcs.UI.ChartFlag.DRAW_GRID_BORDER)) {
        ...
        var t_axis_surface = new Cairo.ImageSurface (Cairo.Format.ARGB32,
                                                     allocation.width,
                                                     allocation.height);
        var t_axis_ctx = new Dcs.UI.AxisView (t_axis_surface);
        ...
        var y_axis_surface = new Cairo.ImageSurface (Cairo.Format.ARGB32,
                                                     allocation.width,
                                                     allocation.height);
        var y_axis_ctx = new Dcs.UI.AxisView (y_axis_surface);
        ...
        if ((parent as Dcs.UI.StripChart).flags.is_set (Dcs.UI.ChartFlag.DRAW_TITLE)) {
            ...
            cr.show_text ((parent as Dcs.UI.StripChart).title);
        }

        if (t_axis.flags.is_set (Dcs.UI.AxisFlag.DRAW_LABEL)) {
            if (t_axis.flags.is_set (Dcs.UI.AxisFlag.ROTATE_LABEL)) {
                cr.save ();
        ...
        if (y_axis.flags.is_set (Dcs.UI.AxisFlag.DRAW_LABEL)) {
            if (y_axis.flags.is_set (Dcs.UI.AxisFlag.ROTATE_LABEL)) {
                cr.save ();
                var font_extents = Cairo.FontExtents ();
                cr.font_extents (out font_extents);
                var text_extents = Cairo.TextExtents ();
        ...
        foreach (var trace in traces.values) {
            ...
            //var data = (trace as Dcs.UI.StripChartTrace).window.to_array ();
            Dcs.Point[] data = new Dcs.Point[(trace as Dcs.UI.StripChartTrace).window_size + 1];
            //for (var i = 0; i < data.length - 1; i++) {
            for (var i = 0; i < data.length; i++) {
                var point = (trace as Dcs.UI.StripChartTrace).window.get (i);
                data[i] = new Dcs.Point (0.0, point.y);
            }
            ...
            /*var color = Gdk.RGBA ();*/
            /*color.parse ((trace as Dcs.UI.StripChartTrace).color_spec);*/
            var color = (trace as Dcs.UI.StripChartTrace).color;

            switch ((trace as Dcs.UI.StripChartTrace).draw_type) {
                case Dcs.UI.TraceDrawType.BAR:
                    var stencil = new Dcs.UI.Bar (trace_surface);
                    stencil.set_line_width ((trace as Dcs.UI.StripChartTrace).line_weight);
                    ...
                    if (y_origin > grid_h)
                        ...
                        y_origin = 0;
                    stencil.draw (data, new Dcs.Point (0.0, y_origin), true);
                    cr.set_operator (Cairo.Operator.OVER);
                    cr.set_source_surface (stencil.get_target (), grid_x, grid_y);
                    cr.paint ();
                    break;
                case Dcs.UI.TraceDrawType.LINE:
                    var stencil = new Dcs.UI.Line (trace_surface);
                    stencil.set_line_width ((trace as Dcs.UI.StripChartTrace).line_weight);
                    stencil.set_source_rgba (color.red, color.green, color.blue, color.alpha);
                    stencil.draw (data);
                    cr.set_operator (Cairo.Operator.OVER);
                    ...
                    cr.paint ();
                    break;
                case Dcs.UI.TraceDrawType.POLYLINE:
                    var stencil = new Dcs.UI.Polyline (trace_surface);
                    stencil.set_line_width ((trace as Dcs.UI.StripChartTrace).line_weight);
                    stencil.set_source_rgba (color.red, color.green, color.blue, color.alpha);
                    stencil.draw (data);
                    cr.set_operator (Cairo.Operator.OVER);
                    ...
                    cr.paint ();
                    break;
                case Dcs.UI.TraceDrawType.SCATTER:
                    var stencil = new Dcs.UI.Scatter (trace_surface);
                    stencil.set_line_width ((trace as Dcs.UI.StripChartTrace).line_weight);
                    stencil.set_source_rgba (color.red, color.green, color.blue, color.alpha);
                    stencil.draw (data);
                    cr.set_operator (Cairo.Operator.OVER);
...
public class Dcs.UI.StripChart : Dcs.UI.CompositeWidget, Dcs.CldAdapter {
    ...
    [GtkChild]
    private Dcs.UI.StripChartCanvas canvas;
    ...
    construct {
        ...
        flags = Dcs.UI.ChartFlag.DRAW_TITLE |
                Dcs.UI.ChartFlag.DRAW_GRID |
                Dcs.UI.ChartFlag.DRAW_GRID_BORDER;
     ...
     */
    public StripChart () {
        var t_axis = new Dcs.UI.Axis ();
        var y_axis = new Dcs.UI.Axis ();
        t_axis.orientation = Dcs.UI.Orientation.HORIZONTAL;
        y_axis.orientation = Dcs.UI.Orientation.VERTICAL;
        add_child (t_axis);
        add_child (y_axis);
        ...
        if (node->type == Xml.ElementType.ELEMENT_NODE &&
            ...
            for (Xml.Node *iter = node->children; iter != null; iter = iter->next) {
                if (iter->name == "property") {
                    ...
                    if (type == "chart-axis") {
                        var axis = new Dcs.UI.Axis.from_xml_node (iter);
                        this.add_child (axis);
                    } else if (type == "stripchart-trace") {
                        var trace = new Dcs.UI.StripChartTrace.from_xml_node (iter);
    ...
    public virtual void offer_cld_object (Cld.Object object) {
        var traces = get_object_map (typeof (Dcs.UI.StripChartTrace));
        foreach (var trace in traces.values) {
            if ((trace as Dcs.UI.StripChartTrace).ch_ref == object.uri) {
                message ("Assigning channel `%s' to `%s'", object.uri, trace.id);
                (trace as Dcs.UI.StripChartTrace).channel = (object as Cld.Channel);
            }
            satisfied = (trace as Dcs.UI.StripChartTrace).channel_isset;
        }
    ...
    public void highlight_trace (string id) {
        var traces = get_object_map (typeof (Dcs.UI.StripChartTrace));
        foreach (var trace in traces.values) {
            (trace as Dcs.UI.StripChartTrace).highlight = false;
            if ((trace as Dcs.UI.StripChartTrace).ch_ref == id) {
                debug ("Chart `%s' highlighting `%s'", this.id, id);
                (trace as Dcs.UI.StripChartTrace).highlight = true;
            }
        }
    }

    private void update_layout () {
        var axes = get_object_map (typeof (Dcs.UI.Axis));
        foreach (var axis in axes.values) {
            if ((axis as Dcs.UI.Axis).orientation == Dcs.UI.Orientation.HORIZONTAL)
                canvas.t_axis = axis as Dcs.UI.Axis;
            else if ((axis as Dcs.UI.Axis).orientation == Dcs.UI.Orientation.VERTICAL)
                canvas.y_axis = axis as Dcs.UI.Axis;
        }
    ...
    protected async void request_data () {
        while (!satisfied) {
            var traces = get_object_map (typeof (Dcs.UI.StripChartTrace));
            foreach (var trace in traces.values) {
                if (!(trace as Dcs.UI.StripChartTrace).channel_isset)
                    request_object ((trace as Dcs.UI.StripChartTrace).ch_ref);
            }
    ...
    private void refresh () {
        ...
        var traces = get_children (typeof (Dcs.UI.StripChartTrace));
        foreach (var trace in traces.values) {
            (trace as Dcs.UI.StripChartTrace).window_size = window_size_max;
            if ((trace as Dcs.UI.StripChartTrace).buffer_size < (window_size_max * stride_min)) {
                (trace as Dcs.UI.StripChartTrace).buffer_size = window_size_max * stride_min;
            }
            (trace as Dcs.UI.StripChartTrace).stride = stride_min;
        }
    ...
    private void update_max_min () {
        ...
        var traces = get_children (typeof (Dcs.UI.StripChartTrace));
        foreach (var trace in traces.values) {
            if ((trace as Dcs.UI.StripChartTrace).window_size > window_size_max) {
                window_size_max = (trace as Dcs.UI.StripChartTrace).window_size;
            }
            if ((trace as Dcs.UI.StripChartTrace).stride < stride_min) {
                stride_min = (trace as Dcs.UI.StripChartTrace).stride;
            }
        }
    }
    ...
    private void spinbutton_delta_t_value_changed_cb () {
        ...
        var traces = get_children (typeof (Dcs.UI.StripChartTrace));
        foreach (var trace in traces.values) {
            (trace as Dcs.UI.StripChartTrace).stride = stride_new;
            if ((trace as Dcs.UI.StripChartTrace).buffer_size < (window_size_max * stride_new)) {
                (trace as Dcs.UI.StripChartTrace).buffer_size = window_size_max * stride_new;
            }
        }
        spinbutton_delta_t.set_value (canvas.t_axis.max - canvas.t_axis.min);
    }
    ...
    private void spinbutton_points_value_changed_cb () {
        ...
        var traces = get_children (typeof (Dcs.UI.StripChartTrace));
        foreach (var trace in traces.values) {
            pts_old = (trace as Dcs.UI.StripChartTrace).window_size;
            if (pts > win_buf) {
                pts = win_buf;
            }
            ...
            (trace as Dcs.UI.StripChartTrace).stride = (int) ((double) win_buf / (double) pts);
            (trace as Dcs.UI.StripChartTrace).window_size = pts;
        }
    }
}
