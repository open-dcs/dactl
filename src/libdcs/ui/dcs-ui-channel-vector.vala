 ...
 */
public class Dcs.UI.ChannelVector : GLib.Object, Dcs.Object,
            ...
            for (Xml.Node *iter = node->children; iter != null; iter = iter->next) {
                if (iter->name == "object"){
                    ...
                    if (type == "channel-vector-element")
                        objects.set (object_id, new Dcs.UI.ChannelVectorElement.from_xml_node (iter));
                }
            }
        }

        var elements = get_object_map (typeof (Dcs.UI.ChannelVectorElement));
        foreach (var element in elements.values) {
            Dcs.UI.ChannelVectorElement e = element as Dcs.UI.ChannelVectorElement;
            references.add (e.chref);
            channel_xvalues.set (e.chref, e.xvalue);
            data += Dcs.SimplePoint ();
    ...
    private void new_value_cb (string uri, double value) {
        ...
        data[ix] = new Dcs.SimplePoint () { x = xvalue, y = value };
    }
