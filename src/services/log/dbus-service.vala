[DBus (name = "org.opendcs.Dcs.Recorder")]
internal class Dcs.Recorder.DBusService : GLib.Object, Dcs.DBusInterface {

    private Dcs.Recorder.Main main;
    private uint name_id;
    private uint connection_id;

    public DBusService (Dcs.Recorder.Main main) {
        this.main = main;
    }

    internal void publish () {
        this.name_id = Bus.own_name (BusType.SESSION,
                                     Dcs.DBusInterface.SERVICE_NAME + ".Recorder",
        ...
        try {
            this.connection_id =
                connection.register_object (
                    Dcs.DBusInterface.OBJECT_PATH + "/Recorder", this
                );
        } catch (Error error) {
