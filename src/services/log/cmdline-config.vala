public class Dcs.Recorder.CmdlineConfig : Dcs.AbstractConfig {
    ...
    private static Once<Dcs.Recorder.CmdlineConfig> _instance;

    public static unowned Dcs.Recorder.CmdlineConfig get_default () {
        return _instance.once (() => { return new Dcs.Recorder.CmdlineConfig (); });
    }

    public static void parse_args (ref unowned string[] args)
                                   throws GLib.OptionError {
        var opt_context = new OptionContext ("DCS Data Recorder Service");
    ...
    public override string get_string (string ns,
        ...
        try {
            unowned string val;
            Dcs.Config.check_property_type (typeof (Dcs.Recorder.CmdlineConfig),
                                            typeof (string),
                                            key);
            @get (key, out val);
            return val;
