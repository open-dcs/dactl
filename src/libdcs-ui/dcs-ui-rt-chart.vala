 ...
 */
public class Dcs.UI.RTChart : Dcs.UI.Chart, Dcs.UI.Settable {
    ...
    private Dcs.UI.SettingsWidget _settings_menu;

    /**
     * {@inheritDoc}
     */
    protected Dcs.UI.SettingsWidget settings_menu {
    ...
    construct {
        ...
        settings_menu = new Dcs.UI.RTChartSettings ();
    ...
    private void bind_to_menu () {
        var menu = settings_menu as Dcs.UI.RTChartSettings;
    ...
    private void start_timer () {
        ...
        timer_id = GLib.Timeout.add (refresh_ms, () => {
            ...
            foreach (var drawable in drawables.values) {
                if (drawable is Dcs.UI.RTTrace)
                    (drawable as Dcs.UI.RTTrace).refresh ();
                else if (drawable is Dcs.UI.RTMultiChannelTrace)
                    (drawable as Dcs.UI.RTMultiChannelTrace).refresh ();
                else if (drawable is Dcs.UI.HeatMap)
                    (drawable as Dcs.UI.HeatMap).refresh ();
            }
            canvas.redraw ();
    ...
    private void connect_notify_signals () {
        ...
        foreach (ParamSpec spec in ocl.list_properties ()) {
            notify[spec.get_name ()].connect ((s, p) => {
                (this as Dcs.UI.RTChart).update_node ();
            });
        }
    ...
    public void highlight_trace (string id) {
        var traces = get_object_map (typeof (Dcs.UI.RTTrace));
        foreach (var trace in traces.values) {
            if (trace is Dcs.UI.RTTrace) {
                (trace as Dcs.UI.RTTrace).highlight = false;
                if ((trace as Dcs.UI.RTTrace).dataseries.ch_ref == id) {
                    debug ("Chart `%s' highlighting `%s'", this.id, id);
                    (trace as Dcs.UI.RTTrace).highlight = true;
                }
            }
        }
    }
...
public class Dcs.UI.RTChartSettings : Gtk.ScrolledWindow, Dcs.SettingsWidget {
    ...
    private void populate_traces () {
        /*var traces = (parent as Dcs.Container).get_object_map (typeof (Dcs.UI.RTTrace));*/
        /*var app = Dcs.UI.Application.get_default ();*/
/*
