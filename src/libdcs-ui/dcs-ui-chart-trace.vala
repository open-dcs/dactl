[Flags]
public enum Dcs.UI.TraceFlag {
    SCROLL_LEFT          = 0x001;

    public Dcs.UI.TraceFlag set (Dcs.UI.TraceFlag flag) {
        return (this | flag);
    }

    public Dcs.UI.TraceFlag unset (Dcs.UI.TraceFlag flag) {
        return (this & ~flag);
    }

    public bool is_set (Dcs.UI.TraceFlag flag) {
        return (flag in this);
    }
}

public enum Dcs.UI.TraceDrawType {
    BAR,
    LINE,
    POLYLINE,
    SCATTER;
...
public class Dcs.UI.Trace : GLib.Object, Dcs.Object,
     ...
     * {@inheritDoc}
     */
    protected virtual Dcs.SimplePoint[] raw_data {
        get {
            return _raw_data;
        }
        set {
     ...
     * {@inheritDoc}
     */
    protected virtual Gee.List<Dcs.Point> pixel_data {
        get {
            return _pixel_data;
        }
        set {
    ...
    public Dcs.UI.TraceFlag flags { get; set; }
    ...
    construct {
        _pixel_data = new Gee.LinkedList<Dcs.Point> ();
    }
    /**
    ...
    private void generate (int w, int h,
        ...
        if (raw_data.length > 2) {
            if (this is Dcs.UI.RTMultiChannelTrace)
                (this as Dcs.UI.RTMultiChannelTrace).update ();
            else
                update ();
        }
    }
    ...
    private void update () {
        ...
        lock (pixel_data) {
            ...
            for (int i = 0; i < points; i++) {
                ...
                var deque = pixel_data as Gee.Deque<Dcs.Point>;
                var point = new Dcs.Point (0, 0);
                ...
                if ((x <= scaled_xdata[0]) || (x >= scaled_xdata[raw_data.length - 1])) {
                    ...
                    if (flags.is_set (Dcs.UI.TraceFlag.SCROLL_LEFT))
                        point.x = width - x;
                    else
                        point.x = x;
    ...
    public void draw (Cairo.Context cr) {
        ...
        switch (draw_type) {
             ...
             *    var stencil = new Dcs.UI.Bar (image_surface);
            ...
            case Dcs.UI.TraceDrawType.LINE:
                var stencil = new Dcs.UI.Line (image_surface);
                stencil.set_line_width (weight);
                stencil.set_source_rgba (color.red, color.green, color.blue, color.alpha);
                stencil.draw (data);
                cr.set_operator (Cairo.Operator.OVER);
            ...
            case Dcs.UI.TraceDrawType.POLYLINE:
                ...
                var stencil = new Dcs.UI.Polyline (image_surface);
                stencil.set_line_width (weight);
                stencil.set_source_rgba (color.red, color.green, color.blue, color.alpha);
                stencil.draw (data);
                cr.set_operator (Cairo.Operator.OVER);
            ...
            case Dcs.UI.TraceDrawType.SCATTER:
                var stencil = new Dcs.UI.Scatter (image_surface);
                stencil.set_line_width (weight);
                stencil.set_source_rgba (color.red, color.green, color.blue, color.alpha);
                stencil.draw (data);
                cr.set_operator (Cairo.Operator.OVER);
