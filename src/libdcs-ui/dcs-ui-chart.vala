...
public enum Dcs.UI.ChartFlag {
    ...
    public Dcs.UI.ChartFlag set (Dcs.UI.ChartFlag flag) {
        return (this | flag);
    }

    public Dcs.UI.ChartFlag unset (Dcs.UI.ChartFlag flag) {
        return (this & ~flag);
    }

    public bool is_set (Dcs.UI.ChartFlag flag) {
        return (flag in this);
    }
}

protected class Dcs.UI.ChartCanvas : Dcs.UI.Canvas {

    public weak Dcs.UI.Axis x_axis { get; set; }
    ...
    public override bool draw (Cairo.Context cr) {
        ...
        var grid = new Dcs.UI.ChartGrid (grid_surface);
...
public class Dcs.UI.Chart : Dcs.UI.CompositeWidget {
    ...
    [GtkChild]
    protected Dcs.UI.ChartCanvas canvas;

    protected Dcs.UI.Axis y_axis;

    protected Dcs.UI.Axis x_axis;
            ...
            for (Xml.Node *iter = node->children; iter != null; iter = iter->next) {
                if (iter->name == "property") {
                    ...
                    switch (iter->get_prop ("name")) {
                        ...
                        case "reverse-x-axis":
                            ...
                            flags = (bool.parse (value))
                                  ? flags.set (Dcs.UI.ChartFlag.REVERSE_X_AXIS)
                                  : flags.unset (Dcs.UI.ChartFlag.REVERSE_X_AXIS);
                            break;
                        case "show-x-axis-label":
                            value = iter->get_content ();
                            flags = (bool.parse (value))
                                  ? flags.set (Dcs.UI.ChartFlag.DRAW_X_AXIS_LABEL)
                                  : flags.unset (Dcs.UI.ChartFlag.DRAW_X_AXIS_LABEL);
                            break;
                        case "rotate-x-axis-label":
                            value = iter->get_content ();
                            flags = (bool.parse (value))
                                  ? flags.set (Dcs.UI.ChartFlag.ROTATE_X_AXIS_LABEL)
                                  : flags.unset (Dcs.UI.ChartFlag.ROTATE_X_AXIS_LABEL);
                            break;
                        case "show-y-axis-label":
                            value = iter->get_content ();
                            flags = (bool.parse (value))
                                  ? flags.set (Dcs.UI.ChartFlag.DRAW_Y_AXIS_LABEL)
                                  : flags.unset (Dcs.UI.ChartFlag.DRAW_Y_AXIS_LABEL);
                            break;
                        case "rotate-y-axis-label":
                            value = iter->get_content ();
                            flags = (bool.parse (value))
                                  ? flags.set (Dcs.UI.ChartFlag.ROTATE_Y_AXIS_LABEL)
                                  : flags.unset (Dcs.UI.ChartFlag.ROTATE_Y_AXIS_LABEL);
                            break;
                        default:
                            break;
                    }
                    ...
                    if (type == "chart-axis") {
                        ...
                        var heatmap = new Dcs.UI.HeatMap.from_xml_node (iter);
    ...
    private void do_flags () {
        var traces = get_object_map (typeof (Dcs.UI.Trace));
        /* Sets the scroll direction of the trace and axis */
        if (flags.is_set (Dcs.UI.ChartFlag.REVERSE_X_AXIS)) {
            x_axis.flags = x_axis.flags.set (Dcs.UI.AxisFlag.REVERSE_ORDER);
            foreach (var trace in traces.values) {
                (trace as Dcs.UI.Trace).flags = (trace as Dcs.UI.Trace).
                                           flags.set (Dcs.UI.TraceFlag.SCROLL_LEFT);
            }
        } else {
            x_axis.flags = x_axis.flags.unset (Dcs.UI.AxisFlag.REVERSE_ORDER);
            foreach (var trace in traces.values) {
                (trace as Dcs.UI.Trace).flags = (trace as Dcs.UI.Trace).
                                           flags.unset (Dcs.UI.TraceFlag.SCROLL_LEFT);
            }
        }
        ...
        if (flags.is_set (Dcs.UI.ChartFlag.DRAW_X_AXIS_LABEL)) {
            grid.attach (lbl_x_axis, 0, 2, 1, 1);
        } else {
            parent.remove (lbl_x_axis);
        }

        if (flags.is_set (Dcs.UI.ChartFlag.ROTATE_X_AXIS_LABEL)) {
        ...
        if (flags.is_set (Dcs.UI.ChartFlag.DRAW_Y_AXIS_LABEL)) {
            grid.attach (lbl_y_axis, 2, 4, 1, 1);
        } else {
            parent.remove (lbl_y_axis);
        }

        if (flags.is_set (Dcs.UI.ChartFlag.ROTATE_Y_AXIS_LABEL)) {
    ...
    private bool draw_more (Gtk.Widget da, Cairo.Context cr) {
        ...
        foreach (var drawable in drawables.values) {
            ...
            (drawable as Dcs.UI.Drawable).generate (w, h, x_min, x_max, y_min, y_max);
