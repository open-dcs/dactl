...
private class Dcs.UI.PnidCanvas : Dcs.UI.Canvas {
    ...
    public Gee.Map<string, Dcs.Object> elements { get; set; }
    ...
    public PnidCanvas (string image_file) {
        ...
        elements = new Gee.TreeMap<string, Dcs.Object> ();
    ...
    public override bool draw (Cairo.Context cr) {
        ...
        foreach (var element in elements.values) {
            if ((element as Dcs.UI.PnidElement).ptype == "pnid-text") {
                try {
                    var xpath = "//svg:text[@id=\"%s\"]/svg:tspan".printf ((element as Dcs.UI.PnidElement).svg_ref);
                    obj = ctx->eval_expression (xpath);
                    var nodes = obj->nodesetval;
                    var node = nodes->item (0);

                    var tokens = (element as Dcs.UI.PnidElement).cld_ref.split (":");
                    var prop = (tokens.length > 1) ? tokens[1] : "value";
                    debug (" > %s : %d", (element as Dcs.UI.PnidElement).cld_ref, tokens.length);

                    /* Check if a property was requested, use the channel scaled value if not */
                    double fval;
                    (element as Dcs.UI.PnidElement).sensor.get (prop, out fval);
                    string val ="";

                    if ((element as Dcs.UI.PnidElement).format == null) {
                        val = "%.2f".printf (fval);
                    } else {
                        val = (element as Dcs.UI.PnidElement).format.printf (fval);
                    }
                    var value = val;
            ...
            } else if ((element as Dcs.UI.PnidElement).ptype == "pnid-rect") {

                // Change the backgound color if in range
                var sp = (element as Dcs.UI.PnidElement).sensor.threshold_sp;
                if (sp != double.NAN) {
                    try {
                        var xpath = "//svg:rect[@id=\"%s\"]".printf ("rect-"+(element as Dcs.UI.PnidElement).svg_ref);
                        obj = ctx->eval_expression (xpath);
                        var nodes = obj->nodesetval;
                        var node = nodes->item (0);
                        var style = node->get_prop ("style");
                        Regex regex = new Regex ("fill:#......");
                        if ((element as Dcs.UI.PnidElement).sensor.threshold_alarm_state) {
                            debug ("1) alarm state: %s", (element as Dcs.UI.PnidElement).sensor.threshold_alarm_state.to_string());
                            style = regex.replace (style, style.length, 0, "fill:#00ff00");
                        } else {
                            debug ("2) alarm state: %s", (element as Dcs.UI.PnidElement).sensor.threshold_alarm_state.to_string());
        ...
        if (zoom) {
            ...
            //var zoom_window = new Dcs.UI.ZoomWindow (zoom_surface);
            //zoom_window.draw ();
...
public class Dcs.UI.Pnid : Dcs.UI.CompositeWidget, Dcs.CldAdapter {
    ...
    public Pnid.from_xml_node (Xml.Node *node) {
        ...
        canvas.elements = get_object_map (typeof (Dcs.UI.PnidElement));
    }
    ...
    public virtual void offer_cld_object (Cld.Object object) {
        ...
        var elements = get_object_map (typeof (Dcs.UI.PnidElement));
        foreach (var element in elements.values) {
            var tokens = (element as Dcs.UI.PnidElement).cld_ref.split (":");

            if (tokens[0] == object.uri) {
                message (" > Assigning `%s' to `%s'", object.uri, element.id);
                (element as Dcs.UI.PnidElement).sensor = (object as Cld.Sensor);
            }
            satisfied = (element as Dcs.UI.PnidElement).sensor_isset;
        }
    ...
    protected async void request_data () {
        ...
        while (!satisfied) {
            ...
            var elements = get_object_map (typeof (Dcs.UI.PnidElement));
            foreach (var element in elements.values) {
                if (!(element as Dcs.UI.PnidElement).sensor_isset) {
                    message ("   > `%s' sensor_isset: %s",
                             element.id, (element as Dcs.UI.PnidElement).sensor_isset.to_string ());
                    var tokens = (element as Dcs.UI.PnidElement).cld_ref.split (":");
