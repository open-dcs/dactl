...
AC_DEFUN([DCS_ADD_STAMP], [
    dcs_stamp_files="$dcs_stamp_files $srcdir/$1"
])

AC_DEFUN([DCS_ADD_VALAFLAGS], [
    VALAFLAGS="${VALAFLAGS:+$VALAFLAGS }$1"
])

dnl DCS_CHECK_PACKAGES(LIST-OF-PACKAGES,
dnl   ACTION-IF-FOUND)
dnl ---------------------------------------
...
AC_DEFUN([DCS_CHECK_PACKAGES], [
    AS_IF([test "x$enable_vala" = "xyes"],
          [VALA_CHECK_PACKAGES([$1],[$2])],
          [$2])
])

dnl _DCS_ADD_PLUGIN_INTERNAL(NAME-OF-PLUGIN,
...
dnl conditional
AC_DEFUN([_DCS_ADD_PLUGIN_INTERNAL], [
    ...
    DCS_ADD_STAMP([src/plugins/$1/libdcs_$2_la_vala.stamp])
    AC_CONFIG_FILES([src/plugins/$1/$1.plugin])
])

dnl DCS_ADD_PLUGIN(NAME-OF-PLUGIN,
dnl   NAME-OF-PLUGIN-FOR-HELP,
dnl   DEFAULT-FOR-ENABLE)
dnl --------------------------------------
dnl Hands off to internal m4.
AC_DEFUN([DCS_ADD_PLUGIN], [
    _DCS_ADD_PLUGIN_INTERNAL([$1],
        m4_translit([$1],[-],[_]),
        [$2],
        [$3])
])

dnl _DCS_DISABLE_PLUGIN_INTERNAL(NAME-OF-PLUGIN)
dnl --------------------------------------
dnl Unset the conditional for building the plugin.
AC_DEFUN([_DCS_DISABLE_PLUGIN_INTERNAL], [
    AM_CONDITIONAL(m4_toupper(build_$1_plugin), false)
    enable_$1_plugin="n/a"
])

dnl DCS_DISABLE_PLUGIN(NAME-OF-PLUGIN)
dnl --------------------------------------
dnl Hands off to internal m4.
AC_DEFUN([DCS_DISABLE_PLUGIN], [
    _DCS_DISABLE_PLUGIN_INTERNAL(m4_translit([$1],[-],[_]))
])

AC_DEFUN([DCS_CHECK_VALA],
[
    AC_ARG_ENABLE([vala],
